...
Within the populate_graph function, we are utilizing this start of a for loop `for i in range(num_users * avg_friendships // 2):`. Given 100 `num_users` and 10 `avg_friendships`, we get 1000 and dividing by 2, we would need to call `add_friendship()` 500 times. Dividing by 2 to remove duplicates.
